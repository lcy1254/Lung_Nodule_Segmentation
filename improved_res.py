...
def preprocess(scan):
    ...
    resize = np.array([scan.pixel_spacing*2, scan.pixel_spacing*2, scan.slice_spacing])
...
def full_preprocess(pid):
    ...
    nib.save(img, os.path.join('/Users/ChaeyoungLee/Downloads','volume-{}.nii.gz'.format(pid)))   #edit path
    labels = nib.Nifti1Image(final_mask, None)
    nib.save(labels, os.path.join('/Users/ChaeyoungLee/Downloads','labels-{}.nii.gz'.format(pid)))    #edit path 
